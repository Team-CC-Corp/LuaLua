local size_t = string.dump(function() end):byte(9)
...
-- dumps a size_t
------------------------------------------------------------------------
function luaU:DumpSize_T(x, D)
	local intcount = size_t / 4
	self:DumpInt(x, D)
	for i=2, intcount do
		-- We're only gonna use those last 4 bytes because I'm lazy and it's unimportant...
		self:DumpInt(0, D)
	end
end
...
-- Dump LASM code
------------------------------------------------------------------------
function luaU:dumpLASM(L, f, w, data, strip)
