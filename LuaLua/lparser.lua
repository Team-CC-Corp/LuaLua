...
function luaY:simpleexp(ls, v)
	...
	--              constructor | FUNCTION body | CLASS | primaryexp
	local c = ls.t.token
	if c == "TK_NUMBER" then
...
function luaY:methodstat(ls, line, v, b, newlocal)
	...
	if newlocal then
		...
		if self:singlevaraux(fs, varname, v, 1) == "VGLOBAL" then
...
function luaY:methodnameandargs(ls, v)
...
function luaY:methodcallglobal(ls, e)
	local fs = ls.fs
	local base = fs.freereg
	luaK:reserveregs(fs, 1)
	local args = {}
	...
	if self:singlevaraux(fs, name, e, 1) == "VGLOBAL" then
		e.info = luaK:stringK(fs, name)  -- info points to global name
	end
	luaK:exp2reg(fs, e, base)

	lua_assert(e.k == "VNONRELOC")
	
	self:init_exp(e, "VCALL", luaK:codeABC(fs, "OP_CALL", base, nparams + 1, 2))
	luaK:fixline(fs, ls.linenumber)
	fs.freereg = base + 1  -- call remove function and arguments and leaves
												 -- (unless changed) one result
end
...
function luaY:classbody(ls, v)
	self:checknext(ls, ":")
	local superClass = {}
	self:expr(ls, superClass)
	luaK:exp2nextreg(ls.fs, superClass)
	local base = superClass.info
	local subclassKey = {}
	self:codestring(ls, subclassKey, "subclassWithClassInstantiator:andObjectInstantiator:")
	luaK:indexed(ls.fs, superClass, subclassKey)
	luaK:exp2reg(ls.fs, superClass, base)

	local static_fs = {}  -- FuncState
				static_fs.upvalues = {}
				static_fs.actvar = {}
	self:open_func(ls, static_fs)
	static_fs.f.lineDefined = ls.linenumber
	static_fs.f.is_vararg = 0
	
	if self:testnext(ls, "TK_STATIC") then
		self:new_localvarliteral(ls, "self", 0) -- only need these if static is actually made
		self:new_localvarliteral(ls, "super", 1)
		self:adjustlocalvars(ls, 2)
		static_fs.f.numparams = 2
		luaK:reserveregs(static_fs, static_fs.nactvar)
		self:chunk(ls)
		self:check_match(ls, "TK_END", "TK_FUNCTION", line)
	end

	static_fs.f.lastlinedefined = ls.linenumber
	self:close_func(ls)
	local static_exp = {}
	self:pushclosure(ls, static_fs, static_exp)
	luaK:exp2reg(ls.fs, static_exp, base + 1)

	local object_fs = {}
				object_fs.upvalues = {}
				object_fs.actvar = {}
	self:open_func(ls, object_fs)
	object_fs.f.lineDefined = ls.linenumber
	object_fs.f.is_vararg = 0

	self:new_localvarliteral(ls, "self", 0)
	self:new_localvarliteral(ls, "super", 1)
	self:adjustlocalvars(ls, 2)
	object_fs.f.numparams = 2
	luaK:reserveregs(object_fs, object_fs.nactvar)
	self:chunk(ls)
	self:check_match(ls, "TK_END", "TK_FUNCTION", line)

	object_fs.f.lastlinedefined = ls.linenumber
	self:close_func(ls)
	local object_exp = {}
	self:pushclosure(ls, object_fs, object_exp)
	luaK:exp2reg(ls.fs, object_exp, base + 2)
	luaK:reserveregs(ls.fs, 3)

	self:init_exp(v, "VCALL", luaK:codeABC(ls.fs, "OP_CALL", base, 3, 2))
	luaK:fixline(ls.fs, ls.linenumber)
	ls.fs.freereg = base + 1
end

function luaY:localclass(ls)
	local v, b = {}, {}  -- expdesc
	local fs = ls.fs
	self:new_localvar(ls, self:str_checkname(ls), 0)
	self:init_exp(v, "VLOCAL", fs.freereg)
...
function luaY:classstat(ls, line)
	self:checknext(ls, "TK_CLASS")
	local v, b = {}, {}
...
function luaY:propertystat(ls, line)
	...
	if self:testnext(ls, "=") then
		localName = self:str_checkname(ls)
		isCustomName = true
	end
	...
	if not isCustomName then
		bl = {}  -- BlockCnt
		self:enterblock(ls.fs, bl, false)
		self:enterlevel(ls)
	end
